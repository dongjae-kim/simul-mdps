...
def main():
    env = CustomEnv('./SUB001_BHV.csv')#gym.make(args.env_name)
    env.seed(args.seed)
    ...
    expert_demo = np.int16(np.genfromtxt('./SUB001_BHV.csv',delimiter=',',usecols=(3,4,5,6,7,17))) # 393 * [each state obs & action1,2]
    expert_data = []
    ...
    for iter in range(args.max_iter_num):
        ...
        for steps in range(demonstrations.shape[0]): 
            ...
            for _ in range(2): 
                ...
                if (steps < 393):
                    next_state, reward, done, _ = env.step(np.argmax(action), gc = demonstrations[steps ,21])
