    ...
    def successor(self, s, a, is_reward):
        ...
        if self.stg == 1:
            if a == 0:
                ...
                next_c = random.choices(population=[self.actions[0], self.actions[1]], weights=[1 - p, p])[0] * 4 + int(
                    self.w / 2)
        ...
        if is_reward == True:
            ...
            if (self.stg == 2):
                if self.mat[self.cti-1, -1] != -1:
                    if reward_bench[(self.mat[self.cti-1, -1] - 6)] == REWARD_LIST[next_s - 32]:
                        reward=REWARD_LIST[next_s-32]
                    else:
                        reward=0
                else:
                    reward=REWARD_LIST[next_s-32]
...
class SRDyna():
    def __init__(self, id, env, loc=(0, 0), **params):
        ...
        # Model
        self.H = params.get('H', np.eye(n_state_actions,
                                        n_state_actions))  # H(s, a) -> Expected discounted state-action occupancy
    ...
    def eps_greedy_policy(self, s, verbose=False, is_reward=False):
        ...
        if greedy:
            ...
            if verbose:
                ...
                # print(self.env.cti)
            if is_reward:
                self.agent_a.append(qpi_all_actions)
        else:
            action = self.env.random_action(s)
            if is_reward:
                qpi_all_actions = np.array([1 - action, action])
                self.agent_a.append(qpi_all_actions)

        return action

    def eps_greedy_policy_likeli(self, s, verbose=False, is_reward=False):
        greedy = np.random.rand() > self.eps
        if greedy:
            sa_index = self.state_action_index(s, 0)
            qpi_all_actions = (self.H[sa_index:sa_index + 2] * self.W).sum(axis=1)
    ...
    def step(self, random_policy=False, verbose=False, learning=True, is_reward=False):
        ...
        if random_policy:  # 첫번째 step
            ...
            a_prime = self.eps_greedy_policy(s_prime, verbose=verbose, is_reward=is_reward)

        likelihood_=1
        # print(len(self.agent_a))
        if done and is_reward and len(self.agent_a)>=2: # self.env.cti>2 and
            likelihood_=self.likelihood(self.agent_a)
            # print(likelihood_)
        r=r*likelihood_
        if len(self.agent_a)>=2:
            self.agent_a = []
    ...
    def step_likeli(self, random_policy=False, verbose=False, learning=True, is_reward=False):
        ...
        if random_policy:  # 첫번째 step
            ...
            a_prime = self.eps_greedy_policy_likeli(s_prime, verbose=verbose, is_reward=is_reward)
    ...
    def likelihood(self, _agent_val):
        # TODO: 맨처음 q-value배울때는 얘가 0인데 그럼 nan? -> 첫번째는 패스(by cti)
        # TODO: 왜 2개씩이 아니라 2 or 3개씩 모이는지 다시 확인 -> 수정완료
        # print(len(_agent_val))
        res = 1
        for i in range(len(_agent_val)):
            if _agent_val[i].all()!=0.:
                norm = _agent_val[i] / np.linalg.norm(_agent_val[i])
                summ = (np.sum(np.exp(norm)))
                if summ == 0:
                    summ += 1e-6
                a_prob = np.exp(norm) / summ
                a = self.env.mat[self.env.cti-1, self.env.stg + i + 1] - 1
                # a = self.env.mat[self.env.cti-1, self.env.stg +3] - 1
                likeli_ = a_prob[a]
                if likeli_ == 0:
                    likeli_ += 1e-6
                res *= likeli_
        ...
        with writer.saving(fig, "./out/%s.mp4" % title, 144):
            for sl in start_locs:
                for trial in range(n_trial_per_loc):
                    self.terminate_stg(reset_state=self.env.state_at_loc(sl))
                    done = False
                    steps = 0
