...
for i_s in range(NUM_SIMUL):
    if not os.path.exists('./bhv_results_pm{0}/{1}/'.format(simnum+1,i_s)):
        os.makedirs('./bhv_results_pm{0}/{1}/'.format(simnum+1,i_s))
    bhv_save = './bhv_results_pm{0}/{1}/SUB{2:03d}_BHV.csv'.format(simnum+1,i_s,subind+1)
    sim_mat = np.zeros((bhv_mat.shape[0],bhv_mat.shape[1]+2))

    # if  os.path.exists(bhv_save):
