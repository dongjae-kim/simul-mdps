...
class MDPWorld(gym.Env):
    ...
    def __init__(self, max_reward_locs=1, world='worlds/mdp.txt', filename='./bhv_results/SUB001_BHV.csv'):
    ...
    def state_at_loc_likeli(self, loc, ignore_reward=False):
        # if not ignore_reward and self.reward_loc(loc):
        #     ro = self.reward_locs.get(loc)
        #     s = ro.get('s')
        # else:
    ...
    def successor(self, s, a):
        ...
        is_reward, reward = self.reward_state(next_s)
    ...
    def successor_likeli(self, s, a, is_reward):
        ...
        if self.stg == 1:
            ...
            self.cti += 1
        next_s = self.state_at_loc_likeli(next_loc)
