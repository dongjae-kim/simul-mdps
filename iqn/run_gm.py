...
def evaluate(eps, frame, eval_runs=5):
    ...
    writer.add_scalar("Reward(eval)", np.mean(reward_batch), int(frame/2))
...
def run(frames=1000, eps_fixed=False, eps_frames=1e6, min_eps=0.01, eval_every=1000, eval_runs=5, worker=1):
    ...
    for frame in range(1, frames+1):
        action, q = agent.act(state, eps)
        next_state, reward, done, _ = envs.step(action[0]) #returns np.stack(obs), np.stack(action) ...
        ...
        if done.any():
            ...
            writer.add_scalar("Average reward(train)", np.mean(scores_window), int(frame/2)*worker)
...
if __name__ == "__main__":
    ...
    if args.save_model:
        torch.save(agent.qnetwork_local.state_dict(), 'pretrained_model_gm/'+args.info+".pth")
