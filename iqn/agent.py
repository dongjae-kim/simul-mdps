...
class IQN_Agent():
    ...
    def step(self, state, action, reward, next_state, done, writer):
        ...
        self.memory.add(state, np.argmax(action), reward, next_state, done)
    ...
    def act(self, state, eps=0., eval=False):
        ...
        if random.random() > eps: # select greedy action if random number is higher than epsilon or noisy network is used!
            ...
            return action, action_values.cpu().data.numpy()[0]
        else:
            if eval:
                action = random.choices(np.arange(self.action_size), k=1)
            else:
                action = random.choices(np.arange(self.action_size), k=self.worker)
            return action, np.array([1-action[0].item(), action[0].item()]) # (left=0, right=1) => left-[1,0], right-[0,1]


    def act_likeli(self, state, eps=0., eval=False):
        """Returns actions for given state as per current policy. Acting only every 4 frames!
        ...
        if random.random() > eps:  # select greedy action if random number is higher than epsilon or noisy network is used!
            ...
            if len(self.state_size) > 1:
                state = torch.from_numpy(state).float().to(
                    self.device)  # .expand(self.K, self.state_size[0], self.state_size[1],self.state_size[2])
            else:
                state = torch.from_numpy(state).float().to(self.device)  # .expand(self.K, self.state_size[0])
            self.qnetwork_local.eval()
            with torch.no_grad():
            ...
            likeli = Q_values/np.sum(Q_values)
            return action, likeli[0,action]
        else:
            if eval:
                action = random.choices(np.arange(self.action_size), k=1)
            else:
