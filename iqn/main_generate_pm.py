...
def run_gen(frames=1000, eps_fixed=False, eps_frames=1e6, min_eps=0.01, eval_every=1000, eval_runs=5, worker=1,bhv_mat = []):
    ...
    for frame in range(1, frames + 1):
        ...
        outdata[simul_env.cti, 6 + simul_env.stg] = action + 1
...
if __name__=="__main__":
    ...
    # bhv_file = './bhv_results0' + str(args.numsim) + '/0/SUB{0:03d}_BHV.csv'.format(args.subid+1)
    ...
    if not os.path.exists("./pretrained_model_gm/" + str(args.numsim) + '/'):
        os.makedirs("./pretrained_model_gm/" + str(args.numsim) + '/')


    agent.qnetwork_local.load_state_dict(torch.load("./pretrained_model_gm/" + str(args.numsim) + '/' +args.info+"_"+ "SUB{0:03d}".format(args.subid+1) +".pth",map_location=torch.device('cpu')))


    # TODO find a way to feed the model to generate behavioral data.
    #

    csv_data = np.int16(np.loadtxt(bhv_file, delimiter=",", dtype=np.float32))

    save_path = './bhv_results0' + str(args.numsim+1) + '/' + str(args.numiter) + '/'
    # parser.add_argument("-subid", "--subid", type=int, default=0,
    #                     help="Subject's ID, default = 0 (0~81)")
    # parser.add_argument("-numsim", "--numsim", type=int, default=0,
    #                     help="Number of iterations, default = 0 (0~99)")
