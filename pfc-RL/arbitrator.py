...
class Arbitrator:
    ...
    def action(self, mf_Q_values, mb_Q_values):
        ...
        for action in range(len(Q) - 1):
            p_action =  exp(Q[action]*self.temperature) / softmax_denom #exp(self.temperature * Q[action]) / exp(self.temperature * Q_sum)
            #print("p_action : ", p_action)
            if random() < p_action:
                return action
        return len(Q) - 1

    def get_likelihood(self, mf_Q_values, mb_Q_values):
        Q = []
        assert len(mf_Q_values) == len(mb_Q_values), "Q value length not match"
        for action in range(len(mf_Q_values)):
    ...
    def get_Rel(self):
        chi_mf = self.mf_rel_estimator.add_pe(rpe) # reliability of model free 
        chi_mb = self.mb_rel_estimator.add_pe(spe) # reliability of model based
        return chi_mf, chi_mb


class Arbitrator_zpe:
    """Arbitrator class
    ...
    def __init__(self, mf_rel_estimator=None, mb_rel_estimator=None, amp_mf_to_mb=AMPLITUDE_MF_TO_MB,
                 amp_mb_to_mf=AMPLITUDE_MB_TO_MF, temperature=SOFTMAX_TEMPERATURE, p_mb=P_MB,
                 max_trans_rate_mf_to_mb=MAX_TRANSITION_RATE_MF_TO_MB,
                 max_trans_rate_mb_to_mf=MAX_TRANSITION_RATE_MB_TO_MF,
                 mf_to_mb_bound=MF_TO_MB_BOUNDARY_CONDITION, mb_to_mf_bound=MB_TO_MF_BOUNDARY_CONDITION):
        self.mf_rel_estimator = mf_rel_estimator if mf_rel_estimator is not None else BayesRelEstimator_zpe()
        self.mb_rel_estimator = mb_rel_estimator if mb_rel_estimator is not None else BayesRelEstimator_zpe()
        self.A_alpha = max_trans_rate_mf_to_mb
        self.A_beta = max_trans_rate_mb_to_mf
        self.B_alpha = log((1 / mf_to_mb_bound) * self.A_alpha - 1)
        self.B_beta = log((1 / mb_to_mf_bound) * self.A_beta - 1)
    ...
    def add_pe(self, rpe, spe, RPEdict_idf, SPEdict_idf):
        chi_mf = self.mf_rel_estimator.add_pe(rpe,RPEdict_idf)  # reliability of model free
        chi_mb = self.mb_rel_estimator.add_pe(spe,SPEdict_idf)  # reliability of model based
        alpha = self.A_alpha / (1 + exp(self.B_alpha * chi_mf))  # transition rate MF->MB
        sum_amp = self.amp_mb_to_mf + self.amp_mf_to_mb
        alpha *= self.amp_mf_to_mb / sum_amp  # multiply by amplitude
        beta = self.A_beta / (1 + exp(self.B_beta * chi_mb))  # transition rate MB->MF
    ...
    def action(self, mf_Q_values, mb_Q_values):
        ...
        assert len(mf_Q_values) == len(mb_Q_values), "Q value length not match"
        for action in range(len(mf_Q_values)):
            Q.append(self.p_mf * mf_Q_values[action] + self.p_mb * mb_Q_values[
                action])  # Q.append(self.p_mf * mf_Q_values[action] + self.p_mb * mf_Q_values[action])
        ...
        for action in range(len(Q) - 1):
            p_action = exp(Q[
                               action] * self.temperature) / softmax_denom  # exp(self.temperature * Q[action]) / exp(self.temperature * Q_sum)
            # print("p_action : ", p_action)
            if random() < p_action:
                return action
        return len(Q) - 1
...
class BayesRelEstimator_zpe:
    """BayesRelEstimator Bayesian Reliability Estimation Class
    """
    MEMORY_SIZE = 10
    CATEGORIES = 3
    THRESHOLD = 0.3
    ...
    def __init__(self, memory_size=MEMORY_SIZE, categories=CATEGORIES, learning_rate=LEARNING_RATE,
                 cond_prob_dist_func=None, target_category=TARGET_CATEGORY):
        """Args:
            memory_size (int): maximum length of memory, which is the 'T' discrete events
            appeared in the paper
        ...
        self.zpe_idf = -1
        # if self.cond_prob_dist_func is None:
        #     self.cond_prob_dist_func = BayesRelEstimator_zpe.DEFAULT_COND_PROB_DIST_FUNC_TEMPLATE(thereshold)

    def init_zpe(self,PE,PEdict):
        num_clus = len(PEdict["CLUSTER_DATA"])
        cluster_wise = []
        for clui in range(num_clus):
            cluster_wise.append( np.mean(np.abs(PE[np.where(PEdict["SAMPLED_CLASS"]==(clui+1))])))
        self.zpe_idf = np.argmin(cluster_wise)


    def add_pe(self, pe,pedict_idf,rel_calc=True):
        if len(self.pe_records) == self.pe_records.maxlen:
            self.pe_records_counter[self.pe_records[0]] -= 1
        if pedict_idf == self.zpe_idf:
            pe_category = 1
        else:
            pe_category = 0
