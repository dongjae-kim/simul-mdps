...
BEHAVIOR_SIMUL_COLUMN = ['episode', 'trial', 'context', 'state 1', 'state 2', 'state 3', 'action 1', 'action 2', \
                         'RT(NaN)', 'RT(NaN)', 'RT(NaN)', 'RT(NaN)', 'RT(NaN)', 'RT(NaN)', 'RT(NaN)','reward received',\
                         'cummulative reward', 'goal']
...
Q_COLUMNS = ['Qforward', 'Qsarsa', 'Qarb','TransitionMap']
PCA_COMPONENTS = 2
ANALYSIS_EXTRA_COLUMNS = ['score', 'rpe', 'spe', 'p_mb']
MODE_IDENTIFIER = 'NN Sequence Control'
MODE_IDENTIFIER2 = 'Opposite Sequence Control'
MODE_IDENTIFIER3 = 'Optimal Sequence Control'
...
class Analysis:
    ...
    """
    def __init__(self):
        self.BEHAVIOR_SIMUL_COLUMN=BEHAVIOR_SIMUL_COLUMN
        self.Q = {}
        self.data = {}
        self.detail = {}
        self.current_data = []
        ...
        self.behavior_simul = []
        self.behavior_simul_df = None
        self.sim_simul = []
        self.sim_simul_df = None
        self.NN = []
        self.NN_df = None

        self.human_action_log_data = {}
        self.task_structure_log_data = {}
        self.state_transition_data = {}
        self.forward_Q_value_data = {}
        self.sarsa_Q_value_data = {}
        self.arb_Q_value_data = {}

        self.human_action_log = []
        self.task_structure_log = []
        self.state_transition = []
        self.forward_Q_value = []
        self.sarsa_Q_value = []
        self.arb_Q_value = []

        self.random_mode_sequence = []
    ...
    def new_simulation(self):
        ...
        self.behavior_simul_df = []
        self.sim_simul_df = []
        self.NN_df = []
        self.current_Q_df = pd.DataFrame(columns=Q_COLUMNS)
    ...
    def complete_simulation(self):
        ...
        self.behavior_simul.append(self.behavior_simul_df)
        self.sim_simul.append(self.sim_simul_df)

    def add_detail_res(self, index, res):
        self.current_detail_df.loc[index] = res

    def add_behavior_simul(self, index, res):
        self.behavior_simul_df.append(res)

    def add_sim_simul(self, index, res):
        self.sim_simul_df.append(res)

    def add_NN(self,res):
        self.NN_df.append(res)
    ...
    def add_log_Q_value(self, _human_action_log, _task_structure_log, _state_transition, _forward_Q_value, _sarsa_Q_value, _arb_Q_value ):
        self.human_action_log.append(_human_action_log)
        self.task_structure_log.append(_task_structure_log)
        self.state_transition.append(_state_transition)
        self.forward_Q_value.append(_forward_Q_value)
        self.sarsa_Q_value.append(_sarsa_Q_value)
        self.arb_Q_value.append(_arb_Q_value) 

    def add_random_mode_sequence(self, _random_mode_list):
        self.random_mode_sequence.append(_random_mode_list)
        #print(self.random_mode_sequence)

    def save_log_Q_value(self, mode):
        self.human_action_log_data[mode] = self.human_action_log.copy()
        self.task_structure_log_data[mode] = self.task_structure_log.copy()
        self.state_transition_data[mode] = self.state_transition.copy()
        self.forward_Q_value_data[mode] = self.forward_Q_value.copy()
        self.sarsa_Q_value_data[mode] = self.sarsa_Q_value.copy()
        self.arb_Q_value_data[mode] = self.arb_Q_value.copy()

    def log_Q_value_to_excel(self, subject_id):
        for mode, subject_list in self.human_action_log_data.items():
            excel_writer = pd.ExcelWriter(self.file_name(mode + ' subject-id ' + str(subject_id) + ' -1. human action sequence') + '.xlsx')
            df = pd.DataFrame(subject_list[subject_id])
            df.to_excel(excel_writer)
            excel_writer.save()
        for mode, subject_list in self.task_structure_log_data.items():
            excel_writer = pd.ExcelWriter(self.file_name(mode + ' subject-id ' + str(subject_id) + ' -2. task structure sequence') + '.xlsx')
            df = pd.DataFrame(subject_list[subject_id])
            df.to_excel(excel_writer)
            excel_writer.save()
        for mode, subject_list in self.state_transition_data.items():
            excel_writer = pd.ExcelWriter(self.file_name(mode + ' subject-id ' + str(subject_id) + ' -3. state transition matrix') + '.xlsx')
            df = pd.DataFrame(subject_list[subject_id])
            df.to_excel(excel_writer)
            excel_writer.save()
        for mode, subject_list in self.forward_Q_value_data.items():
            excel_writer = pd.ExcelWriter(self.file_name(mode + ' subject-id ' + str(subject_id) + ' -4. forward Q value') + '.xlsx')
            df = pd.DataFrame(subject_list[subject_id])
            df.to_excel(excel_writer)
            excel_writer.save()
        for mode, subject_list in self.sarsa_Q_value_data.items():
            excel_writer = pd.ExcelWriter(self.file_name(mode + ' subject-id ' + str(subject_id) + ' -5. sarsa Q value') + '.xlsx')
            df = pd.DataFrame(subject_list[subject_id])
            df.to_excel(excel_writer)
            excel_writer.save()
        for mode, subject_list in self.arb_Q_value_data.items():
            excel_writer = pd.ExcelWriter(self.file_name(mode + ' subject-id ' + str(subject_id) + ' -6. arb Q value') + '.xlsx')
            df = pd.DataFrame(subject_list[subject_id])
            df.to_excel(excel_writer)
            excel_writer.save()

    def random_mode_sequence_to_excel(self):
        #print(self.random_mode_sequence)
        excel_writer = pd.ExcelWriter(self.file_name('random mode sequence') + '.xlsx')
        df = pd.DataFrame(self.random_mode_sequence)
        df.to_excel(excel_writer)
        excel_writer.save()      
    ...
    def cross_mode_summary(self, mode_lst=[mode for mode, _ in MODE_MAP.items()], subject_lst=None, subject_info=None):
        ...
        self._plot_p_mb2(mode_lst, subject_lst, subject_info)

    def cross_mode_summary2(self, mode_lst=[mode for mode, _ in MODE_MAP.items()], subject_lst=None, subject_info=None):
        makedir(RESULTS_FOLDER)
        if subject_lst is not None:
            MODE_MAP[MODE_IDENTIFIER2] = [None, None, 'black']
        self._plot_p_mb2(mode_lst, subject_lst, subject_info)

    def cross_mode_summary3(self, mode_lst=[mode for mode, _ in MODE_MAP.items()], subject_lst=None, subject_info=None):
        makedir(RESULTS_FOLDER)
        if subject_lst is not None:
            MODE_MAP[MODE_IDENTIFIER2] = [None, None, 'black']
            MODE_MAP[MODE_IDENTIFIER] = [None, None, 'saddlebrown']
        self._plot_p_mb2(mode_lst, subject_lst, subject_info)

    def cross_mode_summary4(self, mode_lst=[mode for mode, _ in MODE_MAP.items()], subject_lst=None, subject_info=None, mode = None):
        makedir(RESULTS_FOLDER)
        if subject_lst is not None:
            MODE_MAP[MODE_IDENTIFIER2] = [None, None, 'black']
            MODE_MAP[MODE_IDENTIFIER] = [None, None, 'saddlebrown']
            MODE_MAP[MODE_IDENTIFIER3] = [None, None, 'red']
        self._plot_p_mb2(mode_lst, subject_lst, subject_info, mode)
    ...
    def get_start_point_and_optimal_control_sequence(self, mode, subject_id):
        data_df_lst = self.data[mode]
        data_df = data_df_lst[subject_id]
        detail_df_lst = self.detail[mode]
        detail_df = detail_df_lst[subject_id]
        episode_index = data_df['ctrl_reward'].loc[0.2 * len(data_df):].idxmax() #.rolling(EPISODE_SMOOTH_WINDOW).mean().idxmax()
        #task_structure_df_lst = self.task_structure_log_data[mode]
        #return task_structure_df_lst[subject_id][episode_index*self.trial_separation-1][4:].tolist() + pd.to_numeric(detail_df['action'].loc[episode_index * self.trial_separation :
        #                                            (episode_index + 1) * self.trial_separation - 1], downcast='integer').tolist()
        return pd.to_numeric(detail_df['action'].loc[episode_index * self.trial_separation :
                            (episode_index + 1) * self.trial_separation - 1], downcast='integer').tolist()

    def get_optimal_task_structure_squence_as_frequency(self, mode, subject_id):
        data_df_lst = self.data[mode]
        data_df = data_df_lst[subject_id]
        detail_df_lst = self.detail[mode]
        detail_df = detail_df_lst[subject_id]
        ...
        task_structure_df_lst = self.task_structure_log_data[mode]
        frequency_list = [0, 0, 0, 0] # frequency of [0.5, 0.5], [0.9, .0.1], specific, flexible
        for i in range(self.trial_separation):
            current_task_structure = task_structure_df_lst[subject_id][episode_index*self.trial_separation + i][4:].tolist()
            if current_task_structure[0] == 0.5 and (current_task_structure[2] == 0 or current_task_structure[3] == 0 or current_task_structure[11] ==0):
                frequency_list[0] += 1
            elif current_task_structure[0] == 0.9 and (current_task_structure[2] == 0 or current_task_structure[3] == 0 or current_task_structure[11] ==0):
                frequency_list[1] += 1
            elif current_task_structure[0] == 0.5 and (current_task_structure[2] != 0 and current_task_structure[3] != 0 and current_task_structure[11] !=0):
                frequency_list[2] += 1
            else:
                frequency_list[3] += 1
        stochastic_rate = frequency_list[0]/sum(frequency_list)
        deterministic_rate = frequency_list[1]/sum(frequency_list)
        specific_rate = frequency_list[2]/sum(frequency_list)
        flexible_rate = frequency_list[3]/sum(frequency_list)
        frequency_list = [stochastic_rate, deterministic_rate, specific_rate, flexible_rate]
        return frequency_list



    def optimal_sequence_to_excel(self, mode):
        total_index_optimal_sequence = []
        for i in range(82):
            total_index_optimal_sequence.append( self.get_start_point_and_optimal_control_sequence(mode, i) )#self.get_start_point_and_optimal_control_sequence(mode, i))
        excel_writer = pd.ExcelWriter(self.file_name(mode + ' optimal sequence') + '.xlsx')
        df = pd.DataFrame(total_index_optimal_sequence)
        df.to_excel(excel_writer)
        excel_writer.save()
    ...
    def plot_transfer_compare_learning_curve(self, mode, subject_id, subject_id_of_sequence):
        ...
        if target_val == 'spe':
            plt.plot(original_series, label='Original SPE', color=MODE_MAP[mode][2])
            plt.plot(new_series, label='SPE with Transferred Ctrl', color='black')
            plt.title(mode + ' ' + str(subject_id_of_sequence) + ' applied to ' + str(subject_id) + ' transferred SPE curve')
        else: # traget_val == 'rpe'
            plt.plot(original_series, label='Original RPE', color=MODE_MAP[mode][2])
            plt.plot(new_series, label='RPE with Transferred Ctrl', color='black')
            plt.title(mode + ' ' + str(subject_id_of_sequence) + ' applied to ' + str(subject_id) + ' transferred RPE curve')
        plt.legend(loc='best')
        save_plt_figure(self.file_name(mode + ' ' + str(subject_id_of_sequence) + ' applied to ' + str(subject_id) + ' transferred curve'))

        target_val = MODE_MAP[mode][1]
        if target_val !=None:
            original_series = detail_df[target_val].loc[episode_index * self.trial_separation :
                                       (episode_index + 1) * self.trial_separation - 1].copy().tolist()
            new_series = self.detail[MODE_IDENTIFIER][0][target_val].loc[0:self.trial_separation - 1].copy().tolist()            
            ...
            plt.title(mode + ' ' + str(subject_id_of_sequence) + ' applied to ' + str(subject_id) + ' transferred RPE curve')
            plt.legend(loc='best')
            save_plt_figure(self.file_name(mode + ' ' + str(subject_id_of_sequence) + ' applied to ' + str(subject_id) + ' transferred curve 2'))

    def plot_opposite_compare_learning_curve(self, mode, subject_id):
        data_df = self.data[mode][subject_id]
        detail_df = self.detail[mode][subject_id]
        target_val = MODE_MAP[mode][0]
        episode_index = data_df['ctrl_reward'].loc[0.2 * len(data_df):].idxmax() #.rolling(EPISODE_SMOOTH_WINDOW).mean().idxmax()
        original_series = detail_df[target_val].loc[episode_index * self.trial_separation :
                                       (episode_index + 1) * self.trial_separation - 1].copy().tolist()
        opposite_series = self.detail[MODE_IDENTIFIER2][0][target_val].loc[0:self.trial_separation - 1].copy().tolist()
        if target_val == 'spe':
            plt.plot(original_series, label='Original SPE', color=MODE_MAP[mode][2])
            plt.plot(opposite_series, label='SPE with Opposite Ctrl', color='black')
            plt.title(mode + ', opposite mode sequence applied to ' + str(subject_id) + ' transferred SPE curve')
        else: # traget_val == 'rpe'
            plt.plot(original_series, label='Original RPE', color=MODE_MAP[mode][2])
            plt.plot(opposite_series, label='RPE with Opposite Ctrl', color='black')
            plt.title(mode + ', opposite mode sequence applied to ' + str(subject_id) + ' transferred RPE curve')
        plt.legend(loc='best')
        save_plt_figure(self.file_name(mode + ', opposite mode sequence applied to ' + str(subject_id) + ' transferred curve'))
        ...
        if target_val !=None:
            ...
            plt.title(mode + ', opposite mode sequence applied to ' + str(subject_id) + ' transferred RPE curve')
            plt.legend(loc='best')
            save_plt_figure(self.file_name(mode + ', opposite mode sequence applied to ' + str(subject_id) + ' transferred curve 2'))

    def plot_opposite_nn_compare_learning_curve(self, mode, subject_id, fair = False):
        data_df = self.data[mode][subject_id]
        detail_df = self.detail[mode][subject_id]
        target_val = MODE_MAP[mode][0]
        episode_index = data_df['ctrl_reward'].loc[0.2 * len(data_df):].idxmax() #.rolling(EPISODE_SMOOTH_WINDOW).mean().idxmax()
        if fair:
            original_series = self.detail[MODE_IDENTIFIER3][0][target_val].loc[0:self.trial_separation - 1].copy().tolist()
        else:
            original_series = detail_df[target_val].loc[episode_index * self.trial_separation :
                                       (episode_index + 1) * self.trial_separation - 1].copy().tolist()
        opposite_series = self.detail[MODE_IDENTIFIER2][0][target_val].loc[0:self.trial_separation - 1].copy().tolist()
        ...
        if target_val == 'spe':
            plt.plot(original_series, label='Original SPE', color=MODE_MAP[mode][2])
            plt.plot(opposite_series, label='SPE with Opposite Ctrl', color='black')
            plt.plot(new_series, label = 'SPE with NN Ctrl', color = MODE_MAP[MODE_IDENTIFIER][2])
            plt.title(mode + ', opposite mode sequence applied to ' + str(subject_id) + ' transferred SPE curve')
        else: # traget_val == 'rpe'
            plt.plot(original_series, label='Original RPE', color=MODE_MAP[mode][2])
            plt.plot(opposite_series, label='RPE with Opposite Ctrl', color='black')
            plt.plot(new_series, label = 'RPE with NN Ctrl', color = MODE_MAP[MODE_IDENTIFIER][2])
            plt.title(mode + ', opposite mode sequence applied to ' + str(subject_id) + ' transferred RPE curve')
        plt.legend(loc='best')
        save_plt_figure(self.file_name(mode + ', opposite and nn mode sequence applied to ' + str(subject_id) + ' transferred curve'))
        ...
        if target_val !=None:
            ...
            plt.title(mode + ', opposite mode sequence applied to ' + str(subject_id) + ' transferred RPE curve')
            plt.legend(loc='best')
            save_plt_figure(self.file_name(mode + ', opposite and nn mode sequence applied to ' + str(subject_id) + ' transferred curve 2'))
    ...
    def _plot_p_mb(self, mode_lst, subject_lst, subject_info):
        ...
        for subject_id in range(num_subjects):
            ...
            for mode in mode_lst:
                ...
                data_df = self.data[mode][subject_id]
                detail_df = self.detail[mode][subject_id]                    
                if mode == MODE_IDENTIFIER or mode == MODE_IDENTIFIER2:
                    episode_index = 0
                else:
                ...
                for sub_index in range(EPISODE_SMOOTH_WINDOW):
                    ...
                    if mode == MODE_IDENTIFIER or mode == MODE_IDENTIFIER2: # this prevent plot of transferred control from being reduced by 50 times
                        sub_index = 0
    ...
    def _plot_p_mb2(self, mode_lst, subject_lst, subject_info, mode):
        num_subjects = len(self.current_data)
        if mode == None:
            target_mode = mode_lst[-1]
        else: 
        ...
        discrepancy = 0
        ax = plt.gca()
        for index in range(len(mode_lst)):
            mode = mode_lst[index]
            subject_id = subject_lst[index]                            
            data_df = self.data[mode][subject_id]
            detail_df = self.detail[mode][subject_id]                    
            if mode == MODE_IDENTIFIER or mode == MODE_IDENTIFIER2 or mode == MODE_IDENTIFIER3:
                episode_index = 0
            else:
                total_episode = len(data_df)
                if BEST_EPISODE: # find out the episode with highest score after half way of the training
            ...
            for sub_index in range(EPISODE_SMOOTH_WINDOW):
                discrete_list = np.zeros(self.trial_separation)
                if mode == MODE_IDENTIFIER or mode == MODE_IDENTIFIER2 or mode == MODE_IDENTIFIER3: # this prevent plot of transferred control from being reduced by 50 times
                    sub_index = 0
                for _, row in detail_df.loc[(sub_index + episode_index) * self.trial_separation : 
                                               (sub_index + 1 + episode_index) * self.trial_separation - 1].iterrows(): 
                    category = ceil((1 - row['p_mb']) / interval_len) - 1 # it is suggested to put model-based on the left, hence use 1-p_mb
